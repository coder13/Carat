...
expressions.SequenceExpression = function (scope, node) {
	node.expressions = node.expressions.map(function (node) {
		return resolveExpression(scope, node);
	});
	return node;
};
...
expressions.Property = ['value'];

expressions.ReturnStatement = expressions.ThrowStatement =
expressions.UnaryExpression = expressions.UpdateExpression = ['argument'];

/*  Binary Expressions: */

expressions.BinaryExpression =
expressions.LogicalExpression = ['left', 'right'];
expressions.ConditionalExpression = ['consequent', 'alternate'];

...
const expressions = module.exports = {
	...
	AssignmentExpression: function (scope, node) {
		let left = resolveExpression(scope, node.left);
		if (!left) {
	...
	*/
	...
	MemberExpression: function (scope, node) {
		...
		if (value.type === 'ObjectExpression' && node.property.type === 'Identifier') {
			if (!value.properties) {
				value.properties = [];
			}
...
const resolveExpression = expressions.resolveExpression = function (scope, node) {
	try {
		...
		if (config.debug) {
			console.error(new Error(chalk.red('Unsupported expression type: ') + node.type));
		}
	} catch (e) {
		if (config.debug) {

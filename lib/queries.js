...
const expressions = require('espect');
...
const value = function (node) {
	return node.type === 'Identifier' && node.value ? node.value : node;
};
...
const isSource = function (scope, node, cb, parent) {
	...
	if (node.source) {
		return cb(node.source, node);
	} else if (node.source === false) {
		return;
	}
...
const handlers = {
	index: function (index) {
		return function (scope, node, cb) {
			...
			let arg = value(node.arguments[index]);
			if (arg) {
	...
	callback: function (cbParam, sourceParam) {
		return function (scope, node, cb) {
			...
			let index = cbParam === 'last' ? node.arguments.length - 1 : cbParam;
			if (node.arguments[index]) {
				let func = value(node.arguments[index]);
...
const Sources = module.exports.Sources = [{
	...
	test: tests.object(ast.ce(ast.me(ast.req('http'), 'createServer'))),
	handle: handlers.callback(0, 0)
},  { // Http.get
	name: 'http.get',
	...
	handle: function (scope, node, cb) {
		let func = value(node.arguments[node.arguments.length - 1]);

	...
	{	name: "^require$",
		handler: function (node, ce) {
			...
			this.resolvePath(file, function (pkg) {
				...
				if (newScope.vars.module && newScope.vars.module.props && newScope.vars.module.props.exports) {
...
var Scope = module.exports = function(parent) {
	parent = parent || {};
	this.parent = parent;
	...
	this.resolveExpression['Identifier'] = function (right, resolve) {
		...
		if (!resolved || !resolve) {
			var isSource = false, isSink = false;
		...
		if (isName(ce.callee.type)) {
			...
			if (resolved) {
				if (isName(resolved.type)) {
					...
					// console.log(scope.pos(right), resolved.type, resolved);
				}
				ce.callee = resolved;
			}
		}
		...
		if (ce.arguments && ce.arguments.length != 0) {
			...
			if (isSink) {
				_.some(callbacks, function (callback) {
					if (name.search(callback.name) != 0) {
						return false;
					}
			...
			ce.arguments.forEach(function handleArg(arg) {
				...
				if (arg.type == 'BinaryExpression' || arg.type == 'ConditionalExpression') {
					...
					if (arg.source) {
						...
						scope.report(isSink ? 'SINK' : right.type, right, arg.value, name);
					}
				}
			});
		}
	...
	this.resolveExpression['AssignmentExpression'] = function (right) {
		...
		assign.names.forEach(function (name) {
			...
			if (name.type == 'Identifier') {
				...
				} // Do this for all except for the first and last
...
Scope.prototype.firstScope = function(name) {
	var scope = this;
	while (!_.has(scope.vars, name)) {
		if (!scope.parent)
			return false;
		scope = scope.parent;
	}
	var r = _.has(scope.vars, name) ? scope : false;
	return r;
};
